...
(() => {
  ...
  function buildCard(item) {
    ...
    // selos (array ou boolean/string)
    const seloNames = [];
    if (Array.isArray(item.selos)) {
      seloNames.push(...item.selos);
    } else if (item.selo) {
      seloNames.push(item.selo === true ? 'rei' : String(item.selo));
    }

    seloNames.forEach((nome) => {
      const seloImg = document.createElement('img');
      seloImg.className = 'selo-rei selo-' + nome;
      seloImg.classList.add('lazy');
      seloImg.loading = 'lazy';
      seloImg.alt = 'Selo ' + nome;
      seloImg.src = PLACEHOLDER_1x1;
      // caminho RELATIVO (importante para GitHub Pages em subpastas)
      seloImg.dataset.src = `./assets/svg/selos/${nome}.webp`;
      imgWrapper.appendChild(seloImg);
    });
